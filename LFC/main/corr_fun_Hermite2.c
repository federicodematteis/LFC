...
int main()
{
    ...
    FILE *corr_bin;
    corr_bin = fopen("../../grafici LFC/corr_bin_hermite2.txt", "wt");
    if( corr_bin ==NULL ) {
        perror("Error in opening file");
        exit(1);
    }
    FILE *corr_mean;
    corr_mean = fopen("../../grafici LFC/corr_mean_hermite2.txt", "wt");
    if( corr_mean ==NULL ) {
        perror("Error in opening file");
        exit(1);
    }
    FILE *variance;
    variance = fopen("../../grafici LFC/variance_hermite2.txt", "wt");
    if( variance ==NULL ) {
        perror("Error in opening file");
        exit(1);
    }
