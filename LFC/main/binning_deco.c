...
int main() {
    ...
    FILE *file_C_binned;
    file_C_binned = fopen("../../../grafici LFC/C_binned.txt", "wt");
    ...
    file_C_ensemble = fopen("../../../grafici LFC/C_mean_ensemble.txt", "wt");
    ...
    FILE *file_Energy;
    file_Energy = fopen("../../../grafici LFC/first_energy_gap.txt", "wt");
    if( file_Energy ==NULL ) {
        perror("Error in opening file");
        exit(1);
    }
    ...
    FILE *jack_var_E;
    jack_var_E = fopen("../../../grafici LFC/Jack_variance.txt", "wt");
    if( jack_var_E ==NULL ) {
        perror("Error in opening file");
        exit(1);
    }
    ...
    for (t_phys=0; t_phys<N;t_phys++){
        ...
        var = var * ((double)Nbin_max-1)/(double)Nbin;
        Var[t_phys]=var;
