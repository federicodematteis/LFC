...
fb=fopen("../../grafici LFC/Gamma_t0_N1024.txt","wt");
...
/*after termalization we use 100 more sweep and for each sweep we compute C -> it has size = N */
FILE *fp;
fp = fopen("../../grafici LFC/correlation_matrix.txt", "wt");
if( fp==NULL ) {
    perror("Error in opening file");
    exit(1);
}
...
for (t_real =0; t_real < 1; t_real++) {

    y_mean = 0;
    double Dbin_max = 4000;
...
/*now we can compute the correlation vector for every sweep */
fc=fopen("../../grafici LFC/Decorrelation&Binning.txt","wt");
...
/*now we can compute the correlation vector for every sweep */
fd=fopen("../../grafici LFC/C_bar.txt","wt");
